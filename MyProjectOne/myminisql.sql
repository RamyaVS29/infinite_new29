...
BEGIN
    ...
    DECLARE @DaysOfOperation VARCHAR(250);

    -- Check if the train is active
    IF EXISTS (SELECT 1 FROM Train WHERE TrainNumber = @TrainNumber AND Status = 'Active')
    BEGIN
        -- Fetch days of operation for the given train number
        SELECT @DaysOfOperation = DaysOfOperation
        FROM Train 
        WHERE TrainNumber = @TrainNumber;

        IF @DaysOfOperation IS NOT NULL
        BEGIN
            PRINT 'We operate on the following days: ' + @DaysOfOperation;
        ...
        ELSE
        BEGIN
            PRINT 'Days of operation information not found for the given train number.';
        END
    END
    ELSE
    BEGIN
