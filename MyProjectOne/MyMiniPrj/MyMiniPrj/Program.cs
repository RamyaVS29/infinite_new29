...
namespace MyMiniPrj
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string connectionString = "Server=ICS-LT-9S4R9K3;Database=MyMiniPrjDB;Integrated Security=True;";
        ...
        static void ViewTrain(string connectionString)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // Print table header
                    Console.WriteLine("--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
                    Console.WriteLine("| TrainNumber | TrainName               | Source     | Destination | TicketClass | TicketPrice | TotalBerths | AvailableBerths | Status  | DepartureTime | ArrivalTime | DaysOfOperation                  | Stops |");
                    Console.WriteLine("--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------");

                    // Print each row in a tabular format
                    while (reader.Read())
                    {
                        Console.WriteLine($"| {reader["TrainNumber"],-11} | {reader["TrainName"],-23} | {reader["Source"],-10} | {reader["Destination"],-12} | {reader["TicketClass"],-11} | {reader["TicketPrice"],-12} | {reader["TotalBerths"],-11} | {reader["AvailableBerths"],-15} | {reader["Status"],-8} | {reader["DepartureTime"],-14} | {reader["ArrivalTime"],-12} | {reader["DaysOfOperation"],-31} | {reader["Stops"],-6} |");
                    }
        ...
        static void BookTicket(string connectionString)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // Retrieve train details including DaysOfOperation
                    SqlCommand checkTrainDetailsCommand = new SqlCommand("SELECT Status, AvailableBerths, DaysOfOperation FROM Train WHERE TrainNumber = @TrainNumber", connection);
                    checkTrainDetailsCommand.Parameters.AddWithValue("@TrainNumber", trainNumber);
                    SqlDataReader trainDetailsReader = checkTrainDetailsCommand.ExecuteReader();

                    if (trainDetailsReader.Read())
                    {
                        string trainStatus = (string)trainDetailsReader["Status"];
                        int availableBerths = (int)trainDetailsReader["AvailableBerths"];
                        string daysOfOperation = (string)trainDetailsReader["DaysOfOperation"];
                        trainDetailsReader.Close();

                        // Display operating days
                        Console.WriteLine($"Train operates on: {daysOfOperation}");
